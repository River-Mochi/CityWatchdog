            ...
            Dictionary<string, int> indexCounts)
        {
            ...
            Dictionary<string, string> entries = new Dictionary<string, string>
            {
                ...
                { m_Settings.GetOptionDescLocaleID(nameof(Setting.TrendTracker)),
                    "在底部工具栏的金钱和人口趋势箭头旁添加数字读数。\n" +
                    "这只是轻量 UI 读数，不会改变城市金钱或人口。" },
                { m_Settings.GetOptionLabelLocaleID(nameof(Setting.TrendDisplayMode)), "趋势显示模式" },
                { m_Settings.GetOptionDescLocaleID(nameof(Setting.TrendDisplayMode)),
                    "选择底部工具栏趋势文字显示每小时数值还是每月数值。\n" +
                    "每月金钱使用收入减支出，人口使用 24 小时预测。" },
                ...
                { m_Settings.GetOptionLabelLocaleID(nameof(Setting.UsageText)),
                    ...
                    "\n" +
                    "<金钱辅助>\n" +
                    "1. 趋势追踪会在金钱和人口趋势箭头旁添加 /h 或 /mo 数值。\n" +
                    "2. 添加金钱和扣除金钱使用金钱快捷键金额。\n" +
                    "3. 自动加钱会在城市加载时监控余额，低于阈值时加钱。\n" +
                    "4. 转换无限金钱存档仅适用于以无限金钱开始的城市，且 <City Watchdog 无法撤销>。\n" +
                    "\n" +
                    "<自定义里程碑>\n" +
